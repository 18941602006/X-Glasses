...
当前除 JPEG 外的 kind **仅封装/原样返回**：没有 ToF 距离解码、有效区判断、命令执行、ACK 关联或对时算法；不得将任意负载当成有效传感器读数/执行成功。下一子任务冻结 HELLO/协商、传感器有效性、request_id/去重/截止/ACK、CLOCK 往返估计合同。保留类型号不表示功能已实现。
