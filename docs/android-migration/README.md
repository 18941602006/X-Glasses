...
当前实现增加地图步行导航：可配置 Photon/Valhalla provider、目的地候选、Android 前台位置、逐步指令、到达附近、偏航重算和 Compose 导航页。Manifest 申请 INTERNET 与前台位置权限，但仍不申请后台位置、CAMERA 或 RECORD_AUDIO；未配置服务地址时不联网且地图运行时不可用。`python tools/check_android_contract.py` 只做静态合同检查，不代替 Gradle、APK、服务兼容或真机。
